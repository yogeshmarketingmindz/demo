...
function ts_wp_title( $title, $sep )
{
    ...
    if ( ( $paged >= 2 || $page >= 2 ) && !is_404() ) {
        $title = "$title $sep " . sprintf( __( 'Page %s', 'themestudio' ), max( $paged, $page ) );
    }
...
// Ensure cart contents update when products are added to the cart via AJAX
    ...
    function woocommerce_header_add_to_cart_fragment( $fragments )
    {
        ...
        <a class="cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>"
           title="<?php _e( 'View your shopping cart', 'themestudio' ); ?>">
            <i class="fa fa-shopping-cart"></i>
      <span
          class="cart-number-items"><?php echo sprintf( _n( '%d', $woocommerce->cart->cart_contents_count, 'themestudio' ) ); ?>
      </span></a>
        <?php
