...
class Custom_WC_Widget_Price_Filter extends WC_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo $before_widget . $before_title .'<h3>'. $title .'</h3>'. $after_title;
		
		if ( get_option( 'permalink_structure' ) == '' )
			$form_action = esc_url( remove_query_arg( array( 'page', 'paged' ), add_query_arg( $wp->query_string, '', home_url( $wp->request ) ) ) );
		else
